...
class model_admincuti extends CI_Model
{
	...
	public function validasiDataValue()
	{
		...
		// $dokumen	   = escape($this->input->post('file_doc_', TRUE));
		// if (count($dokumen) > 0) {
		// 	for ($i = 1; $i <= count($dokumen); $i++) {
		// 		$idAdm = escape($this->input->post('file_doc_[' . $i . ']', TRUE));
		// 		$this->form_validation->set_rules($idAdm, 'Dokumen', 'required|trim');
		// 	}
		// }
	...
	public function insertDataDiriCuti()
	{
		...
		$year	 	   = date('Y');
		$month	 	   = date('m');
		$dirname 	   = 'upload/file/administrasi/cuti/' . $year . '/' . $month . '/' . $nikGet;
	...
	public function insertDataPendukung()
	{
		...
		$year	 	   	= date('Y');
		$month	 	   	= date('m');
		$dirname 	   = 'upload/file/pendukung/cuti/' . $year . '/' . $month . '/';
		if (!is_dir($dirname)) {
			mkdir('./' . $dirname, 0777, TRUE);
		}
		$config = array(
	...
	public function createAdmCuti()
	{
		...
		if (empty($id_adminupload)) {
			...
			if (!$aksiupload) {
				...
				$this->db->insert('ms_adminupload', $data);
				return array('message' => 'SUCCESS', 'namafile' => $imagesPend, 'nama' => $nik);
			}
		} else {
			// ambil direktori administrasi
			$checkFile 		= $this->getDataCutiUpload($id_admincuti);
			$nik 			= $checkFile['nik'];
			$year 			= $checkFile['year'];
			$month 			= $checkFile['month'];
			$dirname 	   = 'upload/file/administrasi/cuti/' . $year . '/' . $month . '/' . $nik;
			...
			if (!$aksiupload) {
				...
				$dirnameunlink 	   			= 'upload/file/administrasi/cuti/' . $yearunlink . '/' . $monthunlink . '/' . $nikunlink . '/' . $namafileunlink;
				unlink($dirnameunlink);
				$this->db->where('id_adminupload', $id_adminupload);
				$this->db->update('ms_adminupload', $data);
	...
	public function createPendCuti()
	{
		$create_by     		= $this->app_loader->current_account();
		$create_date   		= date('Y-m-d H:i:s');
		$create_ip     		= $this->input->ip_address();
	...
	public function insertDataPengajuan()
	{
		...
		$create_ip     	= $this->input->ip_address();
		$nama          	= escape($this->input->post('nama_lengkap', TRUE));
		$id 			= base64_decode($this->input->post('id_admincuti'));
		// var_dump($id);
		// exit;
		...
		$this->db->where('id_admincuti', $id);
		$this->db->update('ms_admincuti', $data);
	...
	public function getDataAdminUpload($id_admincuti)
	{
		$this->db->select('date_format(create_date,"%Y") as year, date_format(create_date,"%m") as month, nama_file');
		$this->db->where('id_admincuti', $id_admincuti);
		$query = $this->db->get('ms_adminupload');
		return $query->result_array();
	}

	public function getDataCutiUpload($id_admincuti)
	{
	...
	public function getDataUnlinkedit($idupload)
	{
		$this->db->select('date_format(a.create_date,"%Y") as year, 
							...
							a.nama_file, 
							b.nik');
		$this->db->from('ms_adminupload a');
		$this->db->join('ms_admincuti b', 'a.id_admincuti = b.id_admincuti', 'INNER');
	...
	public function deleteDataAdmincuti()
	{
		...
		$dataUP = $this->getDataAdminUpload($id_admincuti);
		...
		if (count($dataCT) <= 0)
			...
			foreach ($dataUP as $id) {
				...
				$dirname = 'upload/file/administrasi/cuti/' . $year . '/' . $month . '/' . $nik . '/' . $nama_file;
				unlink($dirname);
				$this->db->where('id_admincuti', $id_admincuti);
				$this->db->delete('ms_adminupload');
			}

			$dirname = 'upload/file/administrasi/cuti/' . $year . '/' . $month . '/' . $nik . '/' . $file_ktp;
			unlink($dirname);
			$this->db->where('id_admincuti', $id_admincuti);
	...
	public function delete_data_dukung($id_admincuti)
	{
		...
		$month 			= $checkFile['month'];
		if ($checkFile != '') {
			$dirname = 'upload/file/pendukung/cuti/' . $year . '/' . $month . '/' . $file_support;
			unlink($dirname);
			$this->db->where('id_admincuti', $id_admincuti);
	...
	public function getDataCuti($id)
	{
		$this->db->select('a.id_admincuti,
							...
							d.nm_layanan');
		$this->db->from('ms_admincuti a');
		$this->db->join('ms_adminupload b', 'a.id_admincuti = b.id_admincuti', 'LEFT');
		...
		$this->db->where('a.id_admincuti', $id);
		$this->db->order_by('a.id_admincuti DESC');
		$query = $this->db->get();
		return $query->row_array();
	}
	/*Fungsi get data edit by id dan url*/
	public function eksportcuti($no_reg)
	{
		$this->db->select('a.id_admincuti,
							a.no_urut,
							a.nama_lengkap,
	...
	public function tampilDataAdmCuti($a)
	{
		$this->db->select('
			a.id_administrasi, 
			a.nama_administrasi, 
			...
			c.nama_lengkap');
		$this->db->from('ms_administrasi a');
		$this->db->join('ms_adminupload b', 'a.id_administrasi = b.id_administrasi AND b.id_admincuti = ' . $a, 'LEFT');
		$this->db->join('ms_admincuti c', 	'c.id_admincuti = b.id_admincuti', 'LEFT');
		$this->db->where('a.id_layanan', 4);
		$this->db->where('a.status', 1);
		$this->db->where('a.jenis_administrasi', 'dokumen utama');
	...
	public function tampilDataPendCuti($a)
	{
		$this->db->select('c.id_admincuti,
							c.no_urut,
							c.nama_lengkap,
							c.nik,
							c.create_date,
		...
		$this->db->from('ms_administrasi a');
		$this->db->join('ms_adminupload b', 'b.id_administrasi = a.id_administrasi', 'LEFT');
		$this->db->join('ms_admincuti c', 'b.id_admincuti = c.id_admincuti', 'INNER');
		$this->db->where('c.id_admincuti',  $a);
		$this->db->where('a.id_layanan', 4);
		$this->db->where('a.jenis_administrasi', 'dokumen pendukung');
		$this->db->order_by('b.id_admincuti DESC');
		$query = $this->db->get();
		return $query->result();
	}
	...
	{
		$this->db->select('*');
		$this->db->from('ms_admincuti');
		$this->db->where('id_admincuti', $a);
		$this->db->order_by('id_admincuti DESC');
		$query = $this->db->get();
		return $query->result();
	}

	public function EditDataPendCuti($a)
	{
		$this->db->select('
			a.id_administrasi, 
			a.nama_administrasi, 
	...
	{
		$id_admincuti 		= $this->input->post('id_admincuti', TRUE);
	...
	public function updateDataUploadKtp()
	{
		...
		if (!$upload) {
			...
			$dirnameunlink 	   			= 'upload/file/administrasi/cuti/' . $year . '/' . $month . '/' . $nik . '/' . $namafile;
			unlink($dirnameunlink);
			$this->db->where('id_admincuti', $id_admincuti);
			$this->db->update('ms_admincuti', $data);
			// untuk unlik saat edit data
	...
	public function updateDataUploadPend()
	{
		...
		$idadmincutipend = $this->input->post('idadmincutipend', TRUE);
		$id_administrasi = $this->input->post('id_administrasi', TRUE);
		// ambil direktori pendukung
		$checkFile1 	= $this->getDataCutiUpload($idadmincutipend);
		$nik			= $checkFile1['nik'];
		$year 			= $checkFile1['year'];
		$month 			= $checkFile1['month'];
		$dirname1	   = 'upload/file/administrasi/cuti/' . $year . '/' . $month . '/' . $nik;
		...
		if (empty($iduploadpend)) {
			...
			// ambil direktori administrasi
			$checkFile 		= $this->getDataCutiUpload($idadmincutipend);
			$nik 			= $checkFile['nik'];
			$year 			= $checkFile['year'];
			$month 			= $checkFile['month'];
			$dirname 	   = 'upload/file/administrasi/cuti/' . $year . '/' . $month . '/' . $nik;
