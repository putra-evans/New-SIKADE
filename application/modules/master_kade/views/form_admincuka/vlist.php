  ...
  <div class="row" id="formParent">
    <div class="col-xs-12 col-sm-12">
      <div class="btn-toolbar" style="margin-bottom: 15px">
        <a type="button" class="btn btn-primary" style="padding:11px 16px;" href="<?= site_url('master-kade/admincuka/create/new-register'); ?>"><b><i class="fa fa-plus"></i> Tambah Baru </b></a>
      </div>
    </div>
    <div class="col-xs-12 col-sm-12">
    ...
    <div class="col-xs-12 col-sm-12">
      <div class="panel panel-success">
        <div class="panel-heading">
          <h4>Daftar Administrasi Cuti Kampanye KDH dan Wakil KDH</h4>
        </div>
...
<div class="modal fade in" id="modalEntryForm" tabindex="-1" role="dialog" aria-labelledby="modalEntryLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" id="frmEntry">
    <div class="modal-content">
      ...
      <?php echo form_open_multipart(site_url('master-kade/admincuka/create-pendukung'), array('id' => 'formEntry')); ?>
      <div class="modal-body" style="padding:15px 15px 5px 15px;">
        <div id="errEntry"></div>

        <div class="row">
          <?php echo form_hidden('id_admincuka', ''); ?>
...
<script type="text/javascript">
  ...
  $(document).on('click', '.btnUpload', function(e) {
    formReset();
    $('#formEntry').attr('action', site + 'master-kade/admincuka/create-pendukung');
    var idAdmin = $(this).data('id');
    $('#modalEntryForm').modal({
      backdrop: 'static'
    });
    $('input[name="id_admincuka"]').val(idAdmin);
    // getDataGedung(token);
  });
  ...
  function getDataListadmincuka() {
    $('#tbladministrasi').dataTable({
      ...
      "ordering": false,
      "ajax": {
        "url": site + "master-kade/admincuka/listview",
        "type": "POST",
        "data": {
      ...
      buttons: {
        ...
        "main": {
          ...
          callback: function(response) {
            if (response) {
              $.ajax({
                url: site + 'master-kade/admincuka/delete',
                type: "POST",
                data: postData,
                dataType: "json",
              }).done(function(data) {
