  ...
  <div class="row" id="formParent">
    <div class="col-xs-12 col-sm-12">
      <div class="btn-toolbar" style="margin-bottom: 15px">
        <a type="button" class="btn btn-primary" style="padding:11px 16px;" href="<?= site_url('master-kade/admincuti/create/new-register'); ?>"><b><i class="fa fa-plus"></i> Tambah Baru </b></a>
      </div>
    </div>
    <div class="col-xs-12 col-sm-12">
    ...
    <div class="col-xs-12 col-sm-12">
      <div class="panel panel-magenta">
        <div class="panel-heading">
          <h4>Daftar Administrasi Cuti Di Luar Tanggungan Negara Bagi KDH/Wakil KDH</h4>
        </div>
...
<div class="modal fade in" id="modalEntryForm" tabindex="-1" role="dialog" aria-labelledby="modalEntryLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" id="frmEntry">
    <div class="modal-content">
      ...
      <?php echo form_open_multipart(site_url('master-kade/admincuti/create-pendukung'), array('id' => 'formEntry')); ?>
      <div class="modal-body" style="padding:15px 15px 5px 15px;">
        <div id="errEntry"></div>

        <div class="row">
          <?php echo form_hidden('id_admincuti', ''); ?>
...
<script type="text/javascript">
  ...
  $(document).on('click', '.btnUpload', function(e) {
    formReset();
    $('#formEntry').attr('action', site + 'master-kade/admincuti/create-pendukung');
    var idAdmin = $(this).data('id');
    $('#modalEntryForm').modal({
      backdrop: 'static'
    });
    $('input[name="id_admincuti"]').val(idAdmin);
    // getDataGedung(token);
  });
  ...
  function getDataPendukung(token) {
    ...
    // $.ajax({
    //   type: 'POST',
    //   url: site + 'master-kade/admincuti/details',
  ...
  function getDataListAdmincuti() {
    $('#tbladministrasi').dataTable({
      ...
      "ordering": false,
      "ajax": {
        "url": site + "master-kade/admincuti/listview",
        "type": "POST",
        "data": {
      ...
      buttons: {
        ...
        "main": {
          ...
          callback: function(response) {
            if (response) {
              $.ajax({
                url: site + 'master-kade/admincuti/delete',
                type: "POST",
                data: postData,
                dataType: "json",
              }).done(function(data) {
