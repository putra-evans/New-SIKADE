...
class Administrasi extends SLP_Controller {
  ...
  {
    parent::__construct();
		$this->load->model(array('model_administrasi' => 'mgd', 'model_master' => 'mmas'));
  }
	...
	public function listview()
	{
		if (!$this->input->is_ajax_request()) {
   		exit('No direct script access allowed');
		    ...
		    $dataList = $this->mgd->get_datatables($param);
	        ...
	        "recordsTotal" => $this->mgd->count_all(),
	        "recordsFiltered" => $this->mgd->count_filtered($param),
	        "data" => $data,
	      );
			}
		...
		if (!$this->input->is_ajax_request()) {
   		exit('No direct script access allowed');
			...
			if(!empty($id_administrasi) AND !empty($session)) {
				$data = $this->mgd->getDataDetailadministrasi($this->encryption->decrypt($id_administrasi));
				$row = array();
				$row['nama_administrasi']		= !empty($data) ? $data['nama_administrasi'] : '';
				$row['type']			= !empty($data) ? $data['id_type'] : '';
				$row['instansi']		= !empty($data) ? $data['id_instansi'] : '';
   		...
   		exit('No direct script access allowed');
			...
			if(!empty($session) AND !empty($id_administrasi)) {
				if($this->mgd->validasiDataValue() == FALSE) {
					...
					$data = $this->mgd->updateDataadministrasi();
					if($data['message'] == 'SUCCESS') {
	...
	public function delete()
	{
		if (!$this->input->is_ajax_request()) {
   		exit('No direct script access allowed');
			...
			$id_administrasi 	= escape($this->input->post('tokenId', TRUE));
			if(!empty($session) AND !empty($id_administrasi)) {
				$data = $this->mgd->deleteDataadministrasi();
				if($data['message'] == 'SUCCESS') {
