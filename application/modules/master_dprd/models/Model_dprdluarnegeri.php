...
class Model_dprdluarnegeri extends CI_Model
{
	...
	public function count_all()
	{
		if ($this->app_loader->is_operator()) {
			$this->db->where('id_dprdluarnegeri', instansi($this->app_loader->current_account()));
		}
		return $this->db->count_all_results('ms_dprdluarnegeri');
	}
	...
	private function _get_datatables_query($param)
	{
		...
		$this->db->select('a.id_dprdluarnegeri,
													a.nama_lengkap,
		 											a.nik,
													a.file_ktp,
		 											a.nik,
		...
		$this->db->from('ms_dprdluarnegeri a');
		$this->db->join('ms_instansi b', 'b.id_instansi = a.id_instansi', 'LEFT');
		$this->db->join('ms_pendukungdprdluarnegeri c', 'c.id_dprdluarnegeri = a.id_dprdluarnegeri', 'LEFT');
		$this->db->order_by('a.id_dprdluarnegeri', 'DESC');
		if ($this->app_loader->is_operator()) {
		...
		$this->db->order_by('a.id_dprdluarnegeri ASC');
	}
	...
	public function insertDataDiriDprdluarnegeri()
	{
		...
		$year	 	   = date('Y');
		$month	 	   = date('m');
		$dirname 	   = 'upload/file/administrasi/dprdluarnegeri/' . $year . '/' . $month . '/' . $nikGet;
		...
		if ($_FILES['file_ktp']['name'] == '') {
			...
			$this->db->insert('ms_dprdluarnegeri', $data);
	...
	public function createAdmDprdluarnegeri()
	{
		$create_by     		= $this->app_loader->current_account();
		$create_date   		= date('Y-m-d H:i:s');
		$create_ip     		= $this->input->ip_address();
		$id_adminupload 	= base64_decode($this->input->post('id_adminupload', TRUE));
		$id_dprdluarnegeri 		= base64_decode($this->input->post('id_dprdluarnegeri', TRUE));
		$id_administrasi 	= base64_decode($this->input->post('id_administrasi', TRUE));
		// ambil direktori pendukung
		$checkFile1 	= $this->getDatadprdluarnegeriupload($id_dprdluarnegeri);
		$nik			= $checkFile1['nik'];
		$year 			= $checkFile1['year'];
		$month 			= $checkFile1['month'];
		$dirname1	   = 'upload/file/administrasi/dprdluarnegeri/' . $year . '/' . $month . '/' . $nik;
		...
		if (empty($id_adminupload)) {
			...
			if (!$aksiupload) {
				...
				$data = array(
					'id_dprdluarnegeri' 		=> $id_dprdluarnegeri,
				...
				$this->db->insert('ms_dprdupload_luarnegeri', $data);
				return array('message' => 'SUCCESS', 'namafile' => $imagesPend, 'nama' => $nik);
			}
		} else {
			// ambil direktori administrasi
			$checkFile 		= $this->getDatadprdluarnegeriupload($id_dprdluarnegeri);
			$nik 			= $checkFile['nik'];
			$year 			= $checkFile['year'];
			$month 			= $checkFile['month'];
			$dirname 	   = 'upload/file/administrasi/dprdluarnegeri/' . $year . '/' . $month . '/' . $nik;
			...
			if (!$aksiupload) {
				...
				$dirnameunlink 	   			= 'upload/file/administrasi/dprdluarnegeri/' . $yearunlink . '/' . $monthunlink . '/' . $nikunlink . '/' . $namafileunlink;
				unlink($dirnameunlink);
				$this->db->where('id_adminupload', $id_adminupload);
				$this->db->update('ms_dprdupload_luarnegeri', $data);
	...
	public function createPenddprdluarnegeri()
	{
		$create_by     		= $this->app_loader->current_account();
		$create_date   		= date('Y-m-d H:i:s');
		$create_ip     		= $this->input->ip_address();
	...
	public function insertDataPengajuan()
	{
		...
		$create_ip     	= $this->input->ip_address();
		$nama          	= escape($this->input->post('nama_lengkap', TRUE));
		$id 			= base64_decode($this->input->post('id_dprdluarnegeri'));
		// var_dump($id);
		// exit;
		...
		$this->db->where('id_dprdluarnegeri', $id);
		$this->db->update('ms_dprdluarnegeri', $data);
	...
	public function insertDataPendukung()
	{
		...
		$id_dprdluarnegeri	= $this->encryption->decrypt(escape($this->input->post('id_dprdluarnegeri', TRUE)));
		$file_pendukung = escape($this->input->post('file_pendukung', TRUE));
		$year	 	   	= date('Y');
		$month	 	   	= date('m');
		$dirname 	   = 'upload/file/pendukung/dprdluarnegeri/' . $year . '/' . $month . '/';
		if (!is_dir($dirname)) {
			mkdir('./' . $dirname, 0777, TRUE);
		}
		$config = array(
		...
		if (!$this->upload->do_upload('file_pendukung')) {
			...
			$this->db->insert('ms_pendukungdprdluarnegeri', $data);
			$updateDataStatus = array(
				'id_dprdluarnegeri' 		=> $id_dprdluarnegeri,
				'status' => 'selesai',
				'keterangan_permohonan'		=> 'selesai',
			);
			$this->db->where('id_dprdluarnegeri', $id_dprdluarnegeri);
			$this->db->update('ms_dprdluarnegeri', $updateDataStatus);
			return array('message' => 'SUCCESS', 'kode' => '', 'nama' => $nama);
		}
	}

	public function getDataAdmindprdpepeng($id_dprdluarnegeri)
	{
		$this->db->where('id_dprdluarnegeri', $id_dprdluarnegeri);
		$query = $this->db->get('ms_dprdluarnegeri');
		return $query->row_array();
	}

	public function getDataAdminUpload($id_dprdluarnegeri)
	{
		$this->db->select('date_format(create_date,"%Y") as year, date_format(create_date,"%m") as month, nama_file');
		$this->db->where('id_dprdluarnegeri', $id_dprdluarnegeri);
		$query = $this->db->get('ms_dprdupload_luarnegeri');
		return $query->result_array();
	}

	public function getDatadprdluarnegeriupload($id_dprdluarnegeri)
	{
	...
	public function getDataUnlinkedit($idupload)
	{
		$this->db->select('date_format(a.create_date,"%Y") as year, 
							...
							a.nama_file, 
							b.nik');
		$this->db->from('ms_dprdupload_luarnegeri a');
		$this->db->join('ms_dprdluarnegeri b', 'a.id_dprdluarnegeri = b.id_dprdluarnegeri', 'INNER');
		$this->db->where('a.id_adminupload', $idupload);
		$query = $this->db->get();
		return $query->row_array();
	}

	public function deleteDatadprdluarnegeri()
	{
		$id_dprdluarnegeri = $this->encryption->decrypt(escape($this->input->post('tokenId', TRUE)));
		...
		//cek data rs by id
		$dataCT = $this->getDataAdmindprdpepeng($id_dprdluarnegeri);
		$dataUP = $this->getDataAdminUpload($id_dprdluarnegeri);
		...
		if (count($dataCT) <= 0)
			...
			foreach ($dataUP as $id) {
				...
				$dirname = 'upload/file/administrasi/dprdluarnegeri/' . $year . '/' . $month . '/' . $nik . '/' . $nama_file;
				unlink($dirname);
				$this->db->where('id_dprdluarnegeri', $id_dprdluarnegeri);
				$this->db->delete('ms_dprdupload_luarnegeri');
			}

			$dirname = 'upload/file/administrasi/dprdluarnegeri/' . $year . '/' . $month . '/' . $nik . '/' . $file_ktp;
			unlink($dirname);
			$this->db->where('id_dprdluarnegeri', $id_dprdluarnegeri);
			$this->db->delete('ms_dprdluarnegeri');
			$this->delete_data_dukung($id_dprdluarnegeri);
			return array('message' => 'SUCCESS', 'nama' => $name);
		}
	}

	public function getDataPendukung($id_dprdluarnegeri)
	{
		$this->db->select('date_format(create_date,"%Y") as year, date_format(create_date,"%m") as month, file_pendukung as file_support');
		$this->db->where('id_dprdluarnegeri', $id_dprdluarnegeri);
		$query = $this->db->get('ms_pendukungdprdluarnegeri');
		return $query->row_array();
	}


	public function delete_data_dukung($id_dprdluarnegeri)
	{
		$checkFile 		= $this->getDataPendukung($id_dprdluarnegeri);
		//delete data file di table
		$file_support 	= $checkFile['file_support'];
		$year 			= $checkFile['year'];
		$month 			= $checkFile['month'];
		if ($checkFile != '') {
			$dirname = 'upload/file/pendukung/dprdluarnegeri/' . $year . '/' . $month . '/' . $file_support;
			unlink($dirname);
			$this->db->where('id_dprdluarnegeri', $id_dprdluarnegeri);
			$this->db->delete('ms_pendukungdprdluarnegeri');
		} else {
	...
	public function getDatadprdluarnegeri($id)
	{
		$this->db->select('a.id_dprdluarnegeri,
							a.no_urut,
							a.nama_lengkap,
							...
							d.nm_layanan');
		$this->db->from('ms_dprdluarnegeri a');
		$this->db->join('ms_dprdupload_luarnegeri b', 'a.id_dprdluarnegeri = b.id_dprdluarnegeri', 'LEFT');
		...
		$this->db->where('a.id_dprdluarnegeri', $id);
		$this->db->order_by('a.id_dprdluarnegeri DESC');
		$query = $this->db->get();
		return $query->row_array();
	}
	public function eksportpdf($no_reg)
	{
		$this->db->select('a.id_dprdluarnegeri,
							...
							d.nm_layanan');
		$this->db->from('ms_dprdluarnegeri a');
		$this->db->join('ms_dprdupload_luarnegeri b', 'a.id_dprdluarnegeri = b.id_dprdluarnegeri', 'INNER');
	...
	public function tampilDataAdmDprdLuarnegeri($a)
	{
		$this->db->select('
							a.id_administrasi, 
							a.nama_administrasi, 
							...
							c.nama_lengkap');
		$this->db->from('ms_administrasi a');
		$this->db->join('ms_dprdupload_luarnegeri b', 'a.id_administrasi = b.id_administrasi AND b.id_dprdluarnegeri = ' . $a, 'LEFT');
		$this->db->join('ms_dprdluarnegeri c', 	'c.id_dprdluarnegeri = b.id_dprdluarnegeri', 'LEFT');
		$this->db->where('a.id_layanan', 8);
	...
	public function tampilDataPendDprdLuarnegeri($a)
	{
		$this->db->select('c.id_dprdluarnegeri,
							c.no_urut,
							c.nama_lengkap,
							c.nik,
							c.create_date,
		...
		$this->db->from('ms_administrasi a');
		$this->db->join('ms_dprdupload_luarnegeri b', 'b.id_administrasi = a.id_administrasi', 'LEFT');
		$this->db->join('ms_dprdluarnegeri c', 'b.id_dprdluarnegeri = c.id_dprdluarnegeri', 'INNER');

		// $this->db->join('ms_admincuti c', 'b.id_dprdluarnegeri = c.id_dprdluarnegeri AND c.id_dprdluarnegeri = ' . $a, 'LEFT');
		$this->db->where('c.id_dprdluarnegeri',  $a);
		$this->db->where('a.id_layanan', 8);
		$this->db->where('a.status', 1);
		$this->db->where('a.jenis_administrasi', 'dokumen pendukung');
		$this->db->order_by('b.id_dprdluarnegeri DESC');
	...
	{
		$this->db->select('*');
		$this->db->from('ms_dprdluarnegeri');
		$this->db->where('id_dprdluarnegeri', $a);
		$this->db->order_by('id_dprdluarnegeri DESC');
		$query = $this->db->get();
		return $query->result();
	}
	public function EditDataPendDPRDLuarnegeri($a)
	{
		$this->db->select('
			a.id_administrasi, 
			a.nama_administrasi, 
	...
	{
		$id_dprdluarnegeri 		= $this->input->post('id_dprdluarnegeri', TRUE);
	...
	public function updateDataUploadKtp()
	{
		...
		if (!$upload) {
			...
			$dirnameunlink 	   			= 'upload/file/administrasi/dprdluarnegeri/' . $year . '/' . $month . '/' . $nik . '/' . $namafile;
			unlink($dirnameunlink);
			$this->db->where('id_dprdluarnegeri', $id_dprdluarnegeri);
			$this->db->update('ms_dprdluarnegeri', $data);
			// untuk unlik saat edit data
	...
	public function updateDataUploadPend()
	{
		...
		$iddprdluarnegeripend 	= $this->input->post('iddprdluarnegeripend', TRUE);
		$id_administrasi = $this->input->post('id_administrasi', TRUE);
		// ambil direktori pendukung
		$checkFile1 	= $this->getDatadprdluarnegeriupload($iddprdluarnegeripend);
		$nik			= $checkFile1['nik'];
		$year 			= $checkFile1['year'];
		$month 			= $checkFile1['month'];
		$dirname1	   = 'upload/file/administrasi/dprdluarnegeri/' . $year . '/' . $month . '/' . $nik;
		...
		if (empty($iduploadpend)) {
			...
			if (!$aksiupload) {
				...
				$imagesPend  = $upload_dataPend['file_name'];
				$data = array(
					'id_dprdluarnegeri' 	=> $iddprdluarnegeripend,
					'id_administrasi' 		=> $id_administrasi,
					'nama_file'				=> $imagesPend,
					'create_by' 			=> $this->db->escape_str($create_by),
					'create_date' 			=> $this->db->escape_str($create_date),
			...
			// ambil direktori administrasi
			$checkFile 		= $this->getDatadprdluarnegeriupload($iddprdluarnegeripend);
			$nik 			= $checkFile['nik'];
			$year 			= $checkFile['year'];
			$month 			= $checkFile['month'];
			$dirname 	   = 'upload/file/administrasi/dprdluarnegeri/' . $year . '/' . $month . '/' . $nik;
