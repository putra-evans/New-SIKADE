...
class Model_dprdpepeng extends CI_Model
{
	...
	public function count_all()
	{
		if ($this->app_loader->is_operator()) {
			$this->db->where('id_dprdpepeng', instansi($this->app_loader->current_account()));
		}
		return $this->db->count_all_results('ms_dprdpepeng');
	}
	...
	private function _get_datatables_query($param)
	{
		...
		$this->db->select('a.id_dprdpepeng,
													a.nama_lengkap,
		 											a.nik,
													a.file_ktp,
		 											a.nik,
		...
		$this->db->from('ms_dprdpepeng a');
		$this->db->join('ms_instansi b', 'b.id_instansi = a.id_instansi', 'LEFT');
		$this->db->join('ms_pendukungdprdpepeng c', 'c.id_dprdpepeng = a.id_dprdpepeng', 'LEFT');
		$this->db->order_by('a.id_dprdpepeng', 'DESC');
		if ($this->app_loader->is_operator()) {
		...
		$this->db->order_by('a.id_dprdpepeng ASC');
	}
	...
	public function insertDataDiriDprdpepeng()
	{
		...
		$year	 	   = date('Y');
		$month	 	   = date('m');
		$dirname 	   = 'upload/file/administrasi/dprdpepeng/' . $year . '/' . $month . '/' . $nikGet;
		...
		if ($_FILES['file_ktp']['name'] == '') {
			...
			$this->db->insert('ms_dprdpepeng', $data);
	...
	public function createAdmDprdpepeng()
	{
		$create_by     		= $this->app_loader->current_account();
		$create_date   		= date('Y-m-d H:i:s');
		$create_ip     		= $this->input->ip_address();
		$id_adminupload 	= base64_decode($this->input->post('id_adminupload', TRUE));
		$id_dprdpepeng 		= base64_decode($this->input->post('id_dprdpepeng', TRUE));
		$id_administrasi 	= base64_decode($this->input->post('id_administrasi', TRUE));
		// ambil direktori pendukung
		$checkFile1 	= $this->getDatadprdpepengupload($id_dprdpepeng);
		$nik			= $checkFile1['nik'];
		$year 			= $checkFile1['year'];
		$month 			= $checkFile1['month'];
		$dirname1	   = 'upload/file/administrasi/dprdpepeng/' . $year . '/' . $month . '/' . $nik;
		...
		if (empty($id_adminupload)) {
			...
			if (!$aksiupload) {
				...
				$this->db->insert('ms_dprdupload_pepeng', $data);
				return array('message' => 'SUCCESS', 'namafile' => $imagesPend, 'nama' => $nik);
			}
		} else {
			// ambil direktori administrasi
			$checkFile 		= $this->getDatadprdpepengupload($id_dprdpepeng);
			$nik 			= $checkFile['nik'];
			$year 			= $checkFile['year'];
			$month 			= $checkFile['month'];
			$dirname 	   = 'upload/file/administrasi/dprdpepeng/' . $year . '/' . $month . '/' . $nik;
			...
			if (!$aksiupload) {
				...
				$dirnameunlink 	   			= 'upload/file/administrasi/dprdpepeng/' . $yearunlink . '/' . $monthunlink . '/' . $nikunlink . '/' . $namafileunlink;
				unlink($dirnameunlink);
				$this->db->where('id_adminupload', $id_adminupload);
				$this->db->update('ms_dprdupload_pepeng', $data);
	...
	public function createPenddprdpepeng()
	{
		$create_by     		= $this->app_loader->current_account();
		$create_date   		= date('Y-m-d H:i:s');
		$create_ip     		= $this->input->ip_address();
	...
	public function insertDataPengajuan()
	{
		...
		$create_ip     	= $this->input->ip_address();
		$nama          	= escape($this->input->post('nama_lengkap', TRUE));
		$id 			= base64_decode($this->input->post('id_dprdpepeng'));
		// var_dump($id);
		// exit;
		...
		$this->db->where('id_dprdpepeng', $id);
		$this->db->update('ms_dprdpepeng', $data);
	...
	public function insertDataPendukung()
	{
		...
		$id_dprdpepeng	= $this->encryption->decrypt(escape($this->input->post('id_dprdpepeng', TRUE)));
		$file_pendukung = escape($this->input->post('file_pendukung', TRUE));
		$year	 	   	= date('Y');
		$month	 	   	= date('m');
		$dirname 	   = 'upload/file/pendukung/dprdpepeng/' . $year . '/' . $month . '/';
		if (!is_dir($dirname)) {
			mkdir('./' . $dirname, 0777, TRUE);
		}
		$config = array(
		...
		if (!$this->upload->do_upload('file_pendukung')) {
			...
			$this->db->insert('ms_pendukungdprdpepeng', $data);
			$updateDataStatus = array(
				'id_dprdpepeng' 		=> $id_dprdpepeng,
				'status' => 'selesai',
				'keterangan_permohonan'		=> 'selesai',
			);
			$this->db->where('id_dprdpepeng', $id_dprdpepeng);
			$this->db->update('ms_dprdpepeng', $updateDataStatus);
			return array('message' => 'SUCCESS', 'kode' => '', 'nama' => $nama);
		}
	}

	public function getDataAdmindprdpepeng($id_dprdpepeng)
	{
		$this->db->where('id_dprdpepeng', $id_dprdpepeng);
		$query = $this->db->get('ms_dprdpepeng');
		return $query->row_array();
	}

	public function getDataAdminUpload($id_dprdpepeng)
	{
		$this->db->select('date_format(create_date,"%Y") as year, date_format(create_date,"%m") as month, nama_file');
		$this->db->where('id_dprdpepeng', $id_dprdpepeng);
		$query = $this->db->get('ms_dprdupload_pepeng');
		return $query->result_array();
	}

	public function getDatadprdpepengupload($id_dprdpepeng)
	{
	...
	public function getDataUnlinkedit($idupload)
	{
		$this->db->select('date_format(a.create_date,"%Y") as year, 
							...
							a.nama_file, 
							b.nik');
		$this->db->from('ms_dprdupload_pepeng a');
		$this->db->join('ms_dprdpepeng b', 'a.id_dprdpepeng = b.id_dprdpepeng', 'INNER');
	...
	{
		$id_dprdpepeng = $this->encryption->decrypt(escape($this->input->post('tokenId', TRUE)));
		...
		//cek data rs by id
		$dataCT = $this->getDataAdmindprdpepeng($id_dprdpepeng);
		$dataUP = $this->getDataAdminUpload($id_dprdpepeng);
		...
		if (count($dataCT) <= 0)
			...
			foreach ($dataUP as $id) {
				...
				$dirname = 'upload/file/administrasi/dprdpepeng/' . $year . '/' . $month . '/' . $nik . '/' . $nama_file;
				unlink($dirname);
				$this->db->where('id_dprdpepeng', $id_dprdpepeng);
				$this->db->delete('ms_dprdupload_pepeng');
			}

			$dirname = 'upload/file/administrasi/dprdpepeng/' . $year . '/' . $month . '/' . $nik . '/' . $file_ktp;
			unlink($dirname);
			$this->db->where('id_dprdpepeng', $id_dprdpepeng);
			$this->db->delete('ms_dprdpepeng');
			$this->delete_data_dukung($id_dprdpepeng);
			return array('message' => 'SUCCESS', 'nama' => $name);
		}
	}

	public function getDataPendukung($id_dprdpepeng)
	{
		$this->db->select('date_format(create_date,"%Y") as year, date_format(create_date,"%m") as month, file_pendukung as file_support');
		$this->db->where('id_dprdpepeng', $id_dprdpepeng);
		$query = $this->db->get('ms_pendukungdprdpepeng');
		return $query->row_array();
	}


	public function delete_data_dukung($id_dprdpepeng)
	{
		$checkFile 		= $this->getDataPendukung($id_dprdpepeng);
		//delete data file di table
		$file_support 	= $checkFile['file_support'];
		$year 			= $checkFile['year'];
		$month 			= $checkFile['month'];
		if ($checkFile != '') {
			$dirname = 'upload/file/pendukung/dprdpepeng/' . $year . '/' . $month . '/' . $file_support;
			unlink($dirname);
			$this->db->where('id_dprdpepeng', $id_dprdpepeng);
			$this->db->delete('ms_pendukungdprdpepeng');
		} else {
	...
	public function getDatadprdpepeng($id)
	{
		$this->db->select('a.id_dprdpepeng,
							a.no_urut,
							a.nama_lengkap,
							...
							d.nm_layanan');
		$this->db->from('ms_dprdpepeng a');
		$this->db->join('ms_dprdupload_pepeng b', 'a.id_dprdpepeng = b.id_dprdpepeng', 'LEFT');
		...
		$this->db->where('a.id_dprdpepeng', $id);
		$this->db->order_by('a.id_dprdpepeng DESC');
		$query = $this->db->get();
		return $query->row_array();
	}
	public function eksportpdf($no_reg)
	{
		$this->db->select('a.id_dprdpepeng,
							...
							d.nm_layanan');
		$this->db->from('ms_dprdpepeng a');
		$this->db->join('ms_dprdupload_pepeng b', 'a.id_dprdpepeng = b.id_dprdpepeng', 'INNER');
	...
	public function tampilDataAdmDprdPepeng($a)
	{
		$this->db->select('
							a.id_administrasi, 
							a.nama_administrasi, 
							...
							c.nama_lengkap');
		$this->db->from('ms_administrasi a');
		$this->db->join('ms_dprdupload_pepeng b', 'a.id_administrasi = b.id_administrasi AND b.id_dprdpepeng = ' . $a, 'LEFT');
		$this->db->join('ms_dprdpepeng c', 	'c.id_dprdpepeng = b.id_dprdpepeng', 'LEFT');
		$this->db->where('a.id_layanan', 6);
		$this->db->where('a.status', 1);
		$this->db->where('a.jenis_administrasi', 'dokumen utama');
		$this->db->order_by('a.id_administrasi', 'ASC');
	...
	public function tampilDataPendDprdPepeng($a)
	{
		$this->db->select('c.id_dprdpepeng,
							c.no_urut,
							c.nama_lengkap,
							c.nik,
							c.create_date,
		...
		$this->db->from('ms_administrasi a');
		$this->db->join('ms_dprdupload_pepeng b', 'b.id_administrasi = a.id_administrasi', 'LEFT');
		$this->db->join('ms_dprdpepeng c', 'b.id_dprdpepeng = c.id_dprdpepeng', 'INNER');

		// $this->db->join('ms_admincuti c', 'b.id_dprdpepeng = c.id_dprdpepeng AND c.id_dprdpepeng = ' . $a, 'LEFT');
		$this->db->where('c.id_dprdpepeng',  $a);
		$this->db->where('a.id_layanan', 6);
		$this->db->where('a.jenis_administrasi', 'dokumen pendukung');
		$this->db->order_by('b.id_dprdpepeng DESC');
	...
	{
		$this->db->select('*');
		$this->db->from('ms_dprdpepeng');
		$this->db->where('id_dprdpepeng', $a);
		$this->db->order_by('id_dprdpepeng DESC');
		$query = $this->db->get();
		return $query->result();
	}

	public function EditDataPendDPRDPepeng($a)
	{
		$this->db->select('
			a.id_administrasi, 
			a.nama_administrasi, 
	...
	{
		$id_dprdpepeng 		= $this->input->post('id_dprdpepeng', TRUE);
	...
	public function updateDataUploadKtp()
	{
		...
		if (!$upload) {
			...
			$dirnameunlink 	   			= 'upload/file/administrasi/dprdpepeng/' . $year . '/' . $month . '/' . $nik . '/' . $namafile;
			unlink($dirnameunlink);
			$this->db->where('id_dprdpepeng', $id_dprdpepeng);
			$this->db->update('ms_dprdpepeng', $data);
			// untuk unlik saat edit data
	...
	public function updateDataUploadPend()
	{
		...
		$iddprdpepengpend = $this->input->post('iddprdpepengpend', TRUE);
		$id_administrasi = $this->input->post('id_administrasi', TRUE);
		// ambil direktori pendukung
		$checkFile1 	= $this->getDatadprdpepengupload($iddprdpepengpend);
		$nik			= $checkFile1['nik'];
		$year 			= $checkFile1['year'];
		$month 			= $checkFile1['month'];
		$dirname1	   = 'upload/file/administrasi/dprdpepeng/' . $year . '/' . $month . '/' . $nik;
		...
		if (empty($iduploadpend)) {
			...
			if (!$aksiupload) {
				...
				$imagesPend  = $upload_dataPend['file_name'];
				$data = array(
					'id_dprdpepeng' 	=> $iddprdpepengpend,
					'id_administrasi' 	=> $id_administrasi,
					'nama_file'			=> $imagesPend,
					'create_by' 		=> $this->db->escape_str($create_by),
					'create_date' 		=> $this->db->escape_str($create_date),
			...
			// ambil direktori administrasi
			$checkFile 		= $this->getDatadprdpepengupload($iddprdpepengpend);
			$nik 			= $checkFile['nik'];
			$year 			= $checkFile['year'];
			$month 			= $checkFile['month'];
			$dirname 	   = 'upload/file/administrasi/dprdpepeng/' . $year . '/' . $month . '/' . $nik;
