...
class Users extends SLP_Controller
{
	...
	{
		parent::__construct();
		$this->load->model(array('model_users' => 'muser', 'model_master' => 'mmas'));
	}
	...
	public function index()
	{
		...
		$this->session_info['nm_instansi']  = $this->mmas->getDataInstansi();
	...
	public function details()
	{
		if (!$this->input->is_ajax_request()) {
			...
			if (!empty($token) and !empty($session)) {
				...
				$row['scan_identitas']	= !empty($data) ? $data['scan_identitas'] : '';
				...
				$result = array('status' => $row['status'], 'message' => $row, 'csrfHash' => $csrfHash);
				// } else {
