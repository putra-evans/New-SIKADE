...
/**
 * Fungsi get id_instansi
 */
if (!function_exists('instansi')) {
  function instansi($username) {
    $ci =& get_instance();
    $ci->load->database();
    $ci->db->where('username', $username);
    $ci->db->where('id_status', 1);
    $ci->db->where('blokir', 0);
    $row = $ci->db->get('xi_sa_users')->row_array();
    $id_instansi = !empty($row) ? $row['id_instansi'] : '';
    return $id_instansi;
  }
}

/**
 * Fungsi get nm_instansi
 */
 if (!function_exists('nm_instansi')){
  function nm_instansi($id) {
    $ci =& get_instance();
    $ci->load->database();
    $ci->db->where('id_instansi', $id);
    $row = $ci->db->get('ms_instansi')->row_array();
    $nm_instansi = !empty($row) ? $row['nm_instansi'] : '';
    return $nm_instansi;
  }
 }
 ...
 * Fungsi get district
 */
if (!function_exists('peralatan')) {
  function peralatan($kode) {
    $ci =& get_instance();
    $ci->load->database();
    $ci->db->where('id_peralatan', $kode);
    $row = $ci->db->get('ms_peralatan')->row_array();
    $nm_village = !empty($row) ? $row['nama_peralatan'] : '';
    return $nm_village;
  }
}
